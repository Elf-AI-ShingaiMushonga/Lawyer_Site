...
def _recover_mfa_for_user_row(user: User, *, disable: bool = False) -> dict[str, object]:
...
def recover_user_mfa(app, email: str, *, disable: bool = False) -> dict[str, object]:
    ...
    with app.app_context():
        ...
        result = _recover_mfa_for_user_row(user, disable=disable)
        db.session.commit()
        return result


def recover_all_users_mfa(app, *, disable: bool = False) -> list[dict[str, object]]:
    with app.app_context():
        users = User.query.order_by(User.id.asc()).all()
        if not users:
            return []
        results = [_recover_mfa_for_user_row(user, disable=disable) for user in users]
        db.session.commit()
        return results
